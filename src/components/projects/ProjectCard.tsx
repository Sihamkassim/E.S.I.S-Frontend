...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, onEdit, onSubmit, onDelete, canDelete }) => {
  ...
  return (
    <div className="border rounded-lg p-4 h-full flex flex-col bg-white shadow-sm">
      {/* Cover / header */}
      {cover && (
        <button
          type="button"
          ...
          {project.media.slice(1,5).map(m => {
            ...
            return (
              <button
                ...
                className="relative aspect-video bg-gray-100 rounded overflow-hidden flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                {isVideo ? (
                  <div className="text-[10px] text-gray-600 font-medium">Video</div>
                ) : (
      ...
      <div className="flex-1 space-y-2">
        <h3 className="text-lg font-semibold leading-snug break-words">{project.title}</h3>
        <p className="text-xs text-gray-500">Slug: {project.slug}</p>
        ...
        {project.tags && project.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1">
            {project.tags.map(t => (
              <span key={t.tag.slug} className="bg-blue-50 text-blue-600 text-xs px-2 py-0.5 rounded-md">#{t.tag.name}</span>
            ))}
          </div>
        )}
        ...
        {project.status === 'CHANGES_REQUESTED' && (
          <div className="mt-2 text-xs text-orange-700 bg-orange-50 border border-orange-200 px-2 py-1 rounded">
      ...
      {editable && (
        <div className="mt-4 flex flex-wrap gap-4 text-sm">
          <button className="text-blue-600 hover:underline" onClick={() => onEdit(project)}>Edit</button>
          <button className="text-green-600 hover:underline" onClick={() => onSubmit(project)}>
            {project.status === 'CHANGES_REQUESTED' ? 'Resubmit' : 'Submit'}
          </button>
          {onDelete && canDelete && (
            <button className="text-red-600 hover:underline" onClick={() => onDelete(project)}>Delete</button>
          )}
        </div>
      )}
